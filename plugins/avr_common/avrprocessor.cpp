...
void AvrProcessor::setRegisters()// get register addresses from lst file
{
    QStringList lineList = fileToStringList( m_dataFile, "AvrProcessor::setRegisters" );
    ...
    foreach( QString line, lineList )
    {
        ...
        if( line.contains("equ ") || line.contains("def "))      // This line contains a definition
        {
            ...
            if( isNumber ) addWatchVar( name, address, "u8" ); // type uint8 // If found a valid address add to map
            //qDebug() << name << address<<"\n";
        }
    }
}
